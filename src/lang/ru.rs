...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("connecting_status", "Подключение к сети omendesk..."),
        ...
        ("login_required_to_connect", "Требуется авторизация для подключения к удалённым устройствам"),
        ...
        ("install_tip", "В некоторых случаях из-за UAC omendesk может работать неправильно на удалённом узле. Чтобы избежать возможных проблем с UAC, нажмите кнопку ниже для установки omendesk в системе."),
        ("Click to upgrade", "Нажмите, чтобы обновить"),
        ("Configure", "Настроить"),
        ("config_acc", "Чтобы удалённо управлять своим рабочим столом, вы должны предоставить omendesk права \"доступа\""),
        ("config_screen", "Для удалённого доступа к рабочему столу вы должны предоставить omendesk права \"снимок экрана\""),
        ...
        ("android_input_permission_tip1", "Чтобы удалённое устройство могло управлять вашим Android-устройством с помощью мыши или нажатий, необходимо разрешить omendesk использовать службу \"Специальные возможности\"."),
        ("android_input_permission_tip2", "Перейдите на соответствующую страницу системных настроек, найдите и войдите в \"Установленные службы\", включите службу \"omendesk Input\"."),
        ...
        ("Turned off", "Отключён"),
        ("Language", "Язык"),
        ("Keep omendesk background service", "Держать в фоне службу omendesk"),
        ...
        ("Show omendesk", "Показать omendesk"),
        ...
        ("config_input", "Чтобы управлять удалённым рабочим столом с помощью клавиатуры, необходимо предоставить omendesk разрешения \"Мониторинг ввода\"."),
        ("config_microphone", "Чтобы разговаривать с удалённой стороной, необходимо предоставить omendesk разрешение \"Запись аудио\"."),
        ...
        ("still_click_uac_tip", "По-прежнему требуется, чтобы удалённый пользователь нажал \"OK\" в окне UAC при запуске omendesk."),
        ...
        ("verify_rustdesk_password_tip", "Подтвердить пароль omendesk"),
        ...
        ("scam_text1", "Если вы разговариваете по телефону с кем-то, кого вы НЕ ЗНАЕТЕ и НЕ ДОВЕРЯЕТЕ, и он просит вас использовать omendesk и запустить его службу, не продолжайте и немедленно прервите разговор."),
        ...
        ("upgrade_omendesk_server_pro_to_{}_tip", "Обновите omendesk Server Pro до версии {} или новее!"),
        ...
        ("selinux_tip", "На вашем устройстве включён SELinux, что может помешать правильной работе omendesk на управляемой стороне."),
        ("Change view", "Вид"),
        ...
        ("powered_by_me", "Основано на omendesk"),
        ...
        ("allow-only-conn-window-open-tip", "Разрешать подключение только при открытом окне omendesk"),
        ...
        ("floating_window_tip", "Помогает поддерживать фоновую службу omendesk"),
        ...
        ("upgrade_remote_omendesk_client_to_{}_tip", "Обновите клиент omendesk до версии {} или новее на удалённой стороне!"),
        ...
        ("allow-insecure-tls-fallback-tip", "По умолчанию omendesk проверяет сертификат сервера на наличие протоколов, использующих TLS.\nЕсли эта функция включена, omendesk пропустит данный этап и продолжит работу в случае неудачной проверки."),
        ("Disable UDP", "Отключить UDP"),
        ("disable-udp-tip", "Определяет, следует ли использовать только TCP.\nЕсли включено, omendesk не будет использовать UDP 21116, вместо него будет использоваться TCP 21116."),
        ("server-oss-not-support-tip", "ПРИМЕЧАНИЕ: в OSS-сервере omendesk эта функция отсутствует."),
        ...
        ("omendesk cannot reach the desktop session on the remote device, check that a desktop session is running and that omendesk can use it", "omendesk не может получить доступ к сеансу рабочего стола на удалённом устройстве, проверьте, что сеанс запущен и доступен omendesk"),
        ...
        ("omendesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old", "omendesk не смог получить пригодный экран от XDG Desktop Portal, библиотека PipeWire может быть слишком старой"),
        ("omendesk could not load a GStreamer component needed for screen capture ({})", "omendesk не удалось загрузить компонент GStreamer, необходимый для захвата экрана ({})"),
