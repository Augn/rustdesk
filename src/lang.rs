    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_rustdesk() {
                ...
                && name != "powered_by_me"
            {
                ...
                if !app_name.contains("omendesk") {
                    s = s.replace("omendesk", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "omendesk" (e.g., "omendesk-Admin"), we need to avoid
                    // replacing "omendesk" within the already-substituted app_name, which would
                    // cause duplication like "omendesk-Admin" -> "omendesk-Admin-Admin".
                    //
