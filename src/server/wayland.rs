...
const WAYLAND_NO_SESSION: &str = "omendesk cannot reach the desktop session on the remote device, check that a desktop session is running and that omendesk can use it";
...
const WAYLAND_NO_USABLE_SCREEN: &str = "omendesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old";
const WAYLAND_GST_UNAVAILABLE: &str =
    "omendesk could not load a GStreamer component needed for screen capture ({})";
...
mod tests {
    ...
    fn a_detail_carrying_message_reduces_back_to_its_key() {
        ...
        assert_eq!(
            gst,
            "omendesk could not load a GStreamer component needed for screen capture ({pipewiresrc})"
        );
